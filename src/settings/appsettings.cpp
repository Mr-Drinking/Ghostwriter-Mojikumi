...
public:
    ...
    QHash<QString, QString> translationFiles;
...
bool AppSettings::setUnifiedFont(const QFont &font)
{
    ...
    const bool usesBundledFont =
        BundledFont::isBundledSelection(unifiedFont.family());

    if (usesBundledFont) {
        unifiedFont.setFamily(BundledFont::familyForLocale(d->locale));
    }
...
bool AppSettings::setLocale(const QString &locale)
{
    ...
    auto applyRegionalBundledFont = [this, d](const QString &languageCode) {
        if (!BundledFont::isBundledSelection(d->editorFont.family())) {
            return;
        }

        QFont regionalFont(d->editorFont);
        regionalFont.setFamily(BundledFont::familyForLocale(languageCode));
        d->editorFont = regionalFont;
        d->previewTextFont = regionalFont;
        d->previewCodeFont = regionalFont;
        // The constructor runs before MainWindow connects this signal; an
        // emission there is harmless. Later language changes update both
        // editor and preview through MainWindow's existing connection.
        emit editorFontChanged(regionalFont);
    };

    const QString fullPath = d->translationFiles.value(locale);

    if (fullPath.isEmpty()) {
        // English is the source language and therefore has no translator in
        // some installations. It is still a valid configured locale.
        if (QLocale(locale).language() == QLocale::English) {
            ...
            d->locale = locale;
            applyRegionalBundledFont(locale);
            return true;
        }
        return false;
    }
...
bool AppSettings::canLoadTranslation(const QString &locale) const
{
    Q_D(const AppSettings);

    const QString fullPath = d->translationFiles.value(locale);
    if (fullPath.isEmpty()) {
        return QLocale(locale).language() == QLocale::English;
    }

    QTranslator translator;
    return translator.load(fullPath);
}
    ...
    : d_ptr(new AppSettingsPrivate())
{
    ...
    const int bundledFontId = QFontDatabase::addApplicationFont(
        BundledFont::ResourcePath);

    const QStringList bundledFamilies = BundledFont::collectionFamilies();
    const bool bundledFontAvailable = (bundledFontId >= 0)
        && std::all_of(
            bundledFamilies.cbegin(),
            bundledFamilies.cend(),
            [bundledFontId](const QString &family) {
                return QFontDatabase::applicationFontFamilies(bundledFontId)
                    .contains(family);
            });

    if (!bundledFontAvailable) {
        qCritical() << "Could not register every regional face in the bundled CJK font.";
    }
    if (!BundledFont::installPrivateFile()) {
        qCritical() << "Could not install the application-private WebEngine font copy.";
    }

    // Determine available translation locales. QStandardPaths alone is not
    // reliable for self-contained packages: desktop/AppImage launchers may
    // omit their environment hook, Windows Craft keeps data below bin/, and
    // macOS relocates share/ into Contents/Resources. Search package-relative
    ...
    QStringList localeRoots {
        ...
        applicationDir.filePath(QStringLiteral("../Resources/share/locale")),
    ...
    for (const QString &dataPath : dataPaths) {
        localeRoots.append(QDir(dataPath).filePath(QStringLiteral("locale")));
    }
    ...
    for (const QString &rootPath : std::as_const(localeRoots)) {
...
#ifdef Q_OS_WIN32
        const QString rootKey = cleanRoot.toCaseFolded();
#else
        const QString rootKey = cleanRoot;
#endif
        if (visitedRoots.contains(rootKey)) {
            continue;
        }
        ...
        for (const QString &locale : localeDirs) {
            ...
            if (QFileInfo(translationFilePath).isFile()
                && !d->translationFiles.contains(locale)) {
                d->translationFiles.insert(locale, translationFilePath);
                d->translationList.append(locale);
            }
        }
    }

    // English is the source language and is a valid choice even in packages
    // that intentionally omit an English .qm file.
    if (!d->translationList.contains(QStringLiteral("en"))) {
        d->translationList.append(QStringLiteral("en"));
    }
    d->translationList.sort(Qt::CaseInsensitive);
    ...
    QSettings appSettings;
    d->locale = appSettings.value(
        constants::GW_LOCALE_KEY, d->initialLocale()).toString();

    const QString defaultFontFamily = bundledFontAvailable
        ? BundledFont::familyForLocale(d->locale)
    ...
    if (storedFontParsed
        && BundledFont::isBundledSelection(storedFont.family())
        && bundledFontAvailable) {
        // Migrate the legacy private SC name and follow the currently
        // configured interface language thereafter.
        storedFont.setFamily(BundledFont::familyForLocale(d->locale));
    } else if (!storedFontParsed
